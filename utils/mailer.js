  ...
  return {
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        <h2>Réinitialisation de votre mot de passe Execo</h2>
...
async function sendResetEmail(email, resetUrl) {
  if (ALLOWLIST.length > 0 && !ALLOWLIST.includes(email)) {
    console.log(
      `[mail] Email non autorisé, non envoyé à ${email}. Lien: ${resetUrl}`,
    );
    return;
  }
  ...
  if (!IS_PROD) {
    console.log(
      `[mail] Environnement hors production, email non envoyé. ${email}: ${resetUrl}`,
    );
    return;
  }
